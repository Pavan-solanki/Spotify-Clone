...
// Function to fetch song list from a folder
async function getSongs(folder) {
    try {
        currFolder = folder;
        ...
        if (songs.length === 0) throw new Error("No songs found in this folder");
        ...
        document.querySelectorAll(".songList li").forEach(li => {
            li.addEventListener("click", () => playMusic(li.querySelector(".info div").textContent.trim()));
        });
...
async function main() {
    ...
    // Get the list of all songs and play the first one
    await getSongs("songs/ncs");
    playMusic(songs[0], true);
